...
tic;
    for ii=1:TV_params.itter_max
        ...
        for j2 = 1:3
            Traction_vector(:,:,:,j2) = fftshift(fft2(ifftshift(Traction_vector(:,:,:,j2))));
        end

        gradient_RI(:) = 0;
        for j1 = 1:3
            for j2 = 1:3
                gradient_RI(:,:,:,j1) = gradient_RI(:,:,:,j1)+Green(:,:,:,j1,j2).*Traction_vector(:,:,:,j2);
            end
        ...
        s_n=TV_FISTA_vector(u_n-(1/alpha)*mean(gradient_RI,3),TV_params.tv_param/alpha,...
