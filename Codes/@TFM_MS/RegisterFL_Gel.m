function [RI_registered, shift_vector, tform] = RegisterFL_Gel(TFM,RIRef, RIMoving, z_interface_plot,RI_bg,Mask_RI_2D)
...
% 1. XY first
    ImRef_XY = max(RIRef(:,:,floor(end/9*5)) .* reshape(z_interface_plot,1,1,[]),[],3) .* Mask_RI_2D_shrink;
    ImRef_XY = TFM.high_pass_filter(max(ImRef_XY - RI_bg,0), [1 1]);
    imMoving_XY = max(RIMoving(:,:,floor(end/9*5):end),[],3) .* remove_edge;
    imMoving_XY = TFM.high_pass_filter(max(imMoving_XY - RI_bg,0), [1 1]);
...
%                     figure, imagesc((C)), axis image off
...
% 2. XZ next
    ImRef_XZ = squeeze(max(RIRef.* Mask_RI_2D_shrink))' ;
%     ImRef_XZ = TFM.high_pass_filter(max(ImRef_XZ - RI_bg,0), [3 1]);
    imMoving_XZ = squeeze(max(RI_registered.* Mask_RI_2D_shrink,[],1))';
%     imMoving_XZ = TFM.high_pass_filter(max(imMoving_XZ - RI_bg,0), [3 1]);
...
%  im1 = squeeze(max(FLRef.I_WF{2},[],1))'; im2 = squeeze(max(FLMoving.I_WF{2},[],1))';
%  figure, imshowpair(ImRef_XZ, imMoving_XZ), pause
